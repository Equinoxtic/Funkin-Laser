...
class PlayState extends MusicBeatState
{
	...
	var extraShitText:FlxText;
	var moreExtraShit:FlxText;
	var moreMoreExtraShit:FlxText;
	...
	public static var perfectChains:Int = 0;
	public var scoreTxt:FlxText;
	public var cockScoreTxt:FlxText;
	public var chainTxt:FlxText;
	public var judgementData:FlxText;
	public var extraSongDets:FlxText;
	public static var ratingFC:String = '';
	var chainTmr:FlxTimer;
	var timeTxt:FlxText;
	var scoreTxtTween:FlxTween;
	var cockScoreTwn:FlxTween;
	...
	override public function create()
	{
		...
		reloadTimeBarColors(UIPrefs.timeBarColors);
		...
		extraShitText = new FlxText(-10 * 2, extraSongDets.y - (20 * 4), FlxG.width, "", 20);
		...
		cockScoreTxt.cameras = [camHUD];
		chainTxt.cameras = [camHUD];
		judgementData.cameras = [camHUD];
		extraSongDets.cameras = [camHUD];
		botplayTxt.cameras = [camHUD];
		extraShitText.cameras = [camHUD];
		moreExtraShit.cameras = [camHUD];
		moreMoreExtraShit.cameras = [camHUD];
	...
	public function reloadHealthBarColors(_prefstr:String, _targprefstr:String) {
		if (_prefstr == _targprefstr) {
	...
	public function reloadTimeBarColors(_usecol:Bool) {
		if (_usecol) {
		...
		*/
		...
		if (ModifierVars.botplay) {
			new FlxTimer().start(0.45, function(tmr:FlxTimer) {
				FlxTween.tween(judgementData, {x: FlxG.width * -2, alpha: 0}, 1.5, {ease: FlxEase.backInOut});
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if (cockScoreTmr != null) cockScoreTmr.active = false;
	...
	*/
	...
	override public function update(elapsed:Float)
	{
		...
		switch(curSong.toLowerCase()) {
			case 'blammed':
				...
				if (curStep >= 512 && curStep < 768) {
					FlxG.camera.shake(0.002, 0.08);
					hudTwn = FlxTween.tween(camHUD, {x: 0 - 2 * Math.cos((currentBeat * 0.25) * Math.PI), y: 0 - 4 * Math.cos((currentBeat * 0.25) * Math.PI), angle: -1 * 1 * Math.sin((currentBeat+1)+300)}, 0.1);
					gameTwn = FlxTween.tween(camGame, {x: 0 - 2 * Math.cos((currentBeat * 0.25) * Math.PI), y: 0 - 4 * Math.cos((currentBeat * 0.25) * Math.PI), angle: -1 * 1 * Math.sin((currentBeat+1)+300)}, 0.1);
				} 
		...
		if (UIPrefs.uiType == "Advanced") {
			var ratingParse:String = "";
			var sustainStr:String = "";
			var altStr:String = "";
			...
			if (Conductor.songPosition/1000 < 0 || curStep < 0 || curBeat < 0) {
				...
				moreMoreExtraShit.text = "Rating: ?" + "\nGained Score: 0" + "\nTotal Notes Hit: 0";
			} else {
				extraShitText.text = "songPos: " + Conductor.songPosition/1000 + "\ncurStep: " + curStep + "\ncurBeat: " + curBeat;
				moreExtraShit.text = "Sustain Note: " + sustainStr + "\nNote Direction: " + dirssss + "\nIs Alt Note: " + altStr;
				moreMoreExtraShit.text = "Rating: " + ratingParse + "\nGained Score: " + score + "\nTotal Notes Hit: " + totalNotesHit;
			}
		}

		if (ClientPrefs.advancedScoring) {
			cockScoreTxt.text = 'x' + adCombo + ' | +' + score;
		}
			...
			if (!paused)
			{
				...
				if(updateTime) {
					...
					if (!ClientPrefs.showSongName) {
						...
						timeTxt.scale.x = 0.5;
						timeTxt.scale.y = 0.5;
						timeTxt.text = PlayState.SONG.song + ' - ' + minutesRemaining + ':' + secondsRemaining;
					}
				}
			}
		...
		if (UIPrefs.strumTailShaderType == "Wiggle on beat" && UIPrefs.strumTailShader) {
			noteBeatWiggle.waveAmplitude = FlxMath.lerp(0, noteBeatWiggle.waveAmplitude, CoolUtil.boundTo(1 - (elapsed * 2), 0, 0.9));
			// noteBeatWiggle.waveFrequency = FlxMath.lerp(0, noteBeatWiggle.waveFrequency, CoolUtil.boundTo(1 - (elapsed * 3.125), 0, 1));
			noteBeatWiggle.waveSpeed = FlxMath.lerp(0, noteBeatWiggle.waveSpeed, CoolUtil.boundTo(elapsed * 2, 0, 0));
		}
	...
	/* public function triggerShaderEvent(_shader:String, _val1:String, _val2:String, _val3:String) {
		switch(_shader) {
			case "Glitch":
				var speed:Float = Std.parseFloat(_val1);
				var freq:Float = Std.parseFloat(_val2);
				var amp:Float = Std.parseFloat(_val3);
				if (UIPrefs.vfxModcharts) {
					glitchShader.waveSpeed = _val1;
					glitchShader.waveFrequency = _val2;
					glitchShader.waveAmplitude = _val3;
				}
		}
		callOnLuas('onShader', [_shader, _val1, _val2, val3]);
	} */
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		var cockSkin:String = UIPrefs.judgementStyle.toLowerCase().replace(' ', '_');

		if (PlayState.isPixelStage) {
			if (UIPrefs.judgementStyle == "VS Camellia") {
		...
		if (UIPrefs.judgementStyle != "FNF") {
			rating.loadGraphic(Paths.image(pixelShitPart1 + cockSkin + '/' + daRating + pixelShitPart2));
		} else {
		...
		*/
		...
		if (adCombo >= 500 && ClientPrefs.advancedScoring) {
			if (ClientPrefs.flashing) {
				FlxG.camera.flash(FlxColor.WHITE, 0.75);
				FlxG.sound.play(Paths.sound('cheer_sounds/mfccheer'), 0.85);
				FlxG.sound.play(Paths.sound('results_sounds/mfc'), 0.85);
			}
			perfectChains++;
			adCombo = 0;
			chainTxt.alpha = 1;
			chainTmr = new FlxTimer().start(1.2, function(tmr:FlxTimer) {
				FlxTween.tween(chainTxt, {alpha: 0}, 0.75, {ease: FlxEase.quartInOut});
			});
		}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				if (UIPrefs.uiType == "Advanced") {
					var dirParse:String = "";
					dirParse = animToPlay.replace('sing', '');
					if (dirParse != null || dirParse != "" || dirParse != " ") {
						dirssss = dirParse;
					} else {
						dirssss = "NONE";
					}
	...
	override function beatHit()
	{
		...
		var songPos = Conductor.songPosition;
		var curBpm = Conductor.bpm;
		var currentBeat = (songPos/150)*(curBpm/75);
		if (curBeat % 1 == 0 && UIPrefs.strumTailShaderType == "Wiggle on beat" && UIPrefs.strumTailShader) {
			if (noteBeatWiggle.waveAmplitude < 0.53 && noteBeatWiggle.waveFrequency < 0.53 && noteBeatWiggle.waveSpeed < currentBeat) {
				var ampfreq:Float = 0.052;
