...
class ExtraPrefsSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					case 'Victory Screen':
						ClientPrefs.allowVictoryScreen = !ClientPrefs.allowVictoryScreen;
					case 'Winning Icons':
						ClientPrefs.winningIcons = !ClientPrefs.winningIcons;
				}
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				switch(options[curSelected]) {
					case 'Note Opacity':
						ClientPrefs.noteOpacity += addFloat;
					case 'Note Tail Opacity':
						ClientPrefs.noteTailOpacity += addFloat;
				}
				reloadValues();
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			case 'Victory Screen':
				daText = "If unchecked, the game will prevent you from the Victory Screen.";
			case 'Winning Icons':
				daText =  "If unchecked, Winning Icons will be disabled until this option gets turned on again.";
			case 'Note Opacity':
				daText = "Set how transparent / opaque your notes should be.";
			case 'Note Tail Opacity':
				daText = "Set how transparent / opaque the note's tails are going to be.";
		}
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					case 'Victory Screen':
						daValue = ClientPrefs.allowVictoryScreen;
					case 'Winning Icons':
						daValue = ClientPrefs.winningIcons;
				}
		...
		for (i in 0...grpTexts.members.length) {
			...
			if(text != null) {
				...
				switch(options[textNumber[i]]) {
					case 'Note Opacity':
						daText = '' + ClientPrefs.noteOpacity;
					case 'Note Tail Opacity':
						daText = '' + ClientPrefs.noteTailOpacity;
				}
